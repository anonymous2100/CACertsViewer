...
public class CsrUtils
{
   ...
   */
  public static String generateCsr(boolean isRsaNotEcc)
      throws NoSuchAlgorithmException, InvalidAlgorithmParameterException, OperatorCreationException, IOException
  {
    ...
    ContentSigner signer = new JcaContentSignerBuilder(isRsaNotEcc ? "SHA256WithRSA" : "SM3withSM2").setProvider(BC).build(privateKey);
  ...
  {
    System.out.println("═══════════════════════════════════════════════════════════════");
    System.out.println("  CSR 生成工具演示");
    System.out.println("═══════════════════════════════════════════════════════════════\n");

    // ── 演示 1：生成 RSA-2048 CSR ──────────────────────────────────────
    System.out.println("【1】生成 RSA-2048 CSR");
    System.out.println("────────────────────────────────────────────────────────────");
    try
    {
      String rsaCsrBase64 = generateCsr(true);
      System.out.println("RSA CSR Base64:\n" + rsaCsrBase64);
    }
    catch(Exception e)
    {
      System.err.println("RSA CSR 生成失败: " + e.getMessage());
      e.printStackTrace();
    }
    ...
    // ── 演示 2：生成 ECC（SM2）CSR ────────────────────────────────────
    System.out.println("【2】生成 ECC（SM2 sm2p256v1）CSR");
    System.out.println("────────────────────────────────────────────────────────────");
    try
    {
      String eccCsrBase64 = generateCsr(false);
      System.out.println("ECC（SM2）CSR Base64:");
      System.out.println(eccCsrBase64);
    }
    catch(Exception e)
    {
      System.err.println("ECC（SM2）CSR 生成失败: " + e.getMessage());
      e.printStackTrace();
    }
  }
}